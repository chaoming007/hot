      ...
      <el-table-column align="center" label="管理" width="220" v-if="hasPerm('role:update') ||hasPerm('role:delete') ">
        <template slot-scope="scope">
          <div v-if="scope.row.permissionLevel<=$store.state.user.permissionLevel">
            ...
            <el-button v-if="hasPerm('role:delete')" type="danger" :disabled="scope.row.id<4" icon="delete" @click="removeRole(scope.$index)">
              删除
            </el-button>
          </div>
        </template>
    ...
    methods: {
        getAllPermisson() {
            ...
            this.api({
                ...
                method: "post",
                data: {
                    permissionLevel: _self.$store.state.user.permissionLevel
                }
            }).then(data => {
